...
| `alpha` → `0.1.7-alpha.1` | **更新，但插件生态还不支持，且实测会坏**：现存插件的 peer 范围里完全不出现 0.1.7，多数只写到 `0.1.6-alpha.1` / `0.1.6-alpha.2`。**新机实测（截至 2026-09-23）：装上 0.1.7-alpha.1 后 DSH 插件市场无法正常运行** |
...
兼容性判定**以主流插件为准**：不对市场里的 2300+ 插件做全量普查，主流插件能用即视为该版本可用。
